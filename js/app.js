...
d.getElementById("servicios").addEventListener("change", e => {
  ...
  if ($input.matches(".cart-item-amount")) {
    ...
    cart.update = true
  } else if ($input.name === "formaPago") {
    cart.formaPago = $input.value
  } else if ($input.name === "vendedor") {
    cart.vendedor = $input.value
  } else if ($input.name === "cartHeaderDate") {
    // console.log("cart.fecha=", new Date(cart.fecha))
    cart.fecha = timestampInputDateToDateEc($input.value)
    console.log("despues cart.fecha=", new Date(cart.fecha))
  }
...
// EJEMPLO A LA CARGA
// w.addEventListener('DOMContentLoaded', searchById)
...
const cartIni = {
  cliente: {
    uid: "",
    descripcion: "DEBE BUSCAR Y SELECCIONAR UN CLIENTE",
    ultimoServicio: null,//TODO: Cuando se guarde la factura hay que actualizar esta fecha
    referidos: 0//TODO: Cuando se registra clientes, se debe actualizar el num referidos buscando por identificacion
  },
  vendedor: null,
  formaPago: null,
  items: [],
...
export function changeCartClient($clienteItem) {
  let descartar = true
  if (cart.valido) {
    descartar = confirm(`Existe una venta pendiente de registrar. Que desea hacer: 
...
function renderCartItems() {
  const $productsContainer = d.getElementById("cart-details-productos"),
    $servicesContainer = d.getElementById("cart-details-servicios"),
...
function renderCartSummary() {
  ...
  if (cart.valido && cart.items.length > 0) {
...
// TODO: Validar campos requeridos de cliente al registro y actualizaciones
// TODO: Asignar/sumar referido al momento del registro del cliente
// TODO: Asignar ultimo servicio del cliente al registrar la venta
